...
| Backend    | `server/Dockerfile.backend` | `3000`      | Runs the legacy monolith image. Prefer the service-specific microservice Dockerfiles for active development. |
...
```bash
docker build -f server/Dockerfile.backend -t pharmfind-backend ./server
